...
export function TypedBodyEditor({ type, value, onChange, variables = [] }: { type: SectionType; value: TypedBody; onChange: (value: TypedBody) => void; variables?: readonly DynamicVariable[] }) {
  ...
  if (type === 'MEDIA') return <RichTextEditor label="Media caption" ariaLabel="Media caption" value={value.caption ?? ''} onChange={(caption) => onChange({ caption })} allowedVariables={variables} enableImages={false} minHeight="min-h-24" />;
  if (type === 'RICH_TEXT') {
